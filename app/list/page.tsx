...
const ListsPage = () => {
  ...
  const deleteList = (id: string) => {
    ...
    localStorage.setItem('lists', JSON.stringify(lists.filter(list => list.id !== id)));
    // Clean up associated list data
    localStorage.removeItem(`todos-${id}`);
  };

  return (
    <div className="h-full flex flex-col items-center p-6" dir={language === 'he' ? 'rtl' : 'ltr'}>
      <div className="w-full max-w-2xl">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold">{translations[language].myLists}</h1>
              ...
              >
                ...
                >
                  ...
                  <p className="text-sm text-gray-500">
                    {translations[language].created} {new Date(list.createdAt).toLocaleDateString()}
                  </p>
                </button>
                <Button
                  variant="ghost"
                  ...
                  className={`text-gray-400 hover:text-red-500 hover:bg-red-50 ${language === 'he' ? 'mr-4' : 'ml-4'}`}
                >
